...
def list_toolchains():
    ...
    Returns:
       `list of tuples`:  Each tuple is an :code:`(architecture, language)` pair that is supported.
    """
       
    return TheToolchainRegistry.get_available_toolchains()
...
class GCCToolchain(Toolchain):
    ...
    def describe(self):
        return f"{self._tool_prefix}gcc toolchain compiling for {self.architecture_name}"
