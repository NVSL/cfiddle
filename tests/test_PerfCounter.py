...
def test_perf_count_easy(mem_loop):
    skip_if_no_perf_counters()
    results = run(mem_loop, "go", map_product(count=[10000]), perf_counters=[CPU_CYCLES])
    assert "CPU_CYCLES" in results.as_dicts()[0]
    print(results.as_df())

def test_perf_count_reset(mem_loop):
    skip_if_no_perf_counters()
    results = run(mem_loop, "go", map_product(count=[10000,10000]), perf_counters=[CPU_CYCLES])
...
def test_perf_count_multiple(mem_loop):
    skip_if_no_perf_counters()
    results = run(mem_loop, "go", map_product(count=[10000]), perf_counters=[CPU_CYCLES, INSTRUCTIONS])
    assert "CPU_CYCLES" in results.as_dicts()[0]
...
def test_perf_count_cache(mem_loop):
    skip_if_no_perf_counters()
    results = run(mem_loop, "go", map_product(count=[10000]), perf_counters=[CacheCounter(L1D, READ, ACCESS)])
